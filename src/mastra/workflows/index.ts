import { embed, EmbedManyResult, Step, Workflow } from '@mastra/core';
...
const embedStep = new Step({
  ...
  execute: async ({ context, mastra }) => {
    if (context.machineContext?.stepResults?.['fetch-content']?.status !== 'success') {
      throw new Error('Fetch content step failed');
    }

    const metadata = context.machineContext?.triggerData.metadata || {};

    const html = context.machineContext?.stepResults?.['fetch-content']?.payload?.content
...
const upsertStep = new Step({
  ...
  execute: async ({ context, mastra }) => {
    ...
    if (context.machineContext?.stepResults?.['embed-content']?.status !== 'success') {
    ...
    const { embeddings, chunks } = context.machineContext?.stepResults?.['embed-content']?.payload
